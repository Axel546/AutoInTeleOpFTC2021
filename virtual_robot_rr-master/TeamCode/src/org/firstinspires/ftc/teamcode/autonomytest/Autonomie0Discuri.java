...
@Autonomous(name = "Autonomie0Discuri", group = "autonomytest")
public class Autonomie0Discuri extends OpMode {
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case PAUSE3:
                if ((clock.seconds() - startPause) > 2.0) {
                    state = State.AWAY4;
                    traj = new TrajectoryBuilder(new Pose2d(69, -22.5, 0), bot.constraints)
                            .strafeTo(new Vector2d(15,-22.5))
                            .build();
                    bot.follower.followTrajectory(traj);
                }
                break;
